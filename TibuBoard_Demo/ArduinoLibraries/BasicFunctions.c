...
void InitADC()
{
	...
	/*Enable GPIO clock*/
	RCC_AHB1PeriphClockCmd(GPIOPIN_CLK[A3], ENABLE);

	/*Configure ADC3 Channel11 pin as analog input*/
	GPIO_InitStructure.GPIO_Pin = GPIOPIN_PIN[A3];
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;

	GPIO_Init(GPIOPIN_PORT[A3], &GPIO_InitStructure);
...
/**
  * @brief  Returns the selected pin state.
  * @param  None.
  * @retval The pin A3 analog value.
  */
uint16_t analogRead()
{
...
{
	return value*((maxOutput-minOutput)/(maxInput-minInput));
}
